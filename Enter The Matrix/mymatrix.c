...
void matrix_info(int id, struct matrix *m) {
    INFO("Matrix 0x%x 0x%x", m, m->data);
...
void handle_list() {
    for(int i=0; i<MAXM; i++) {
        INFO("Matrices 0x%x 0x%x", matrices+i, *(matrices+i));
...
void handle_command(const char *cmdbuf) {
    ...
    if(2 == sscanf(cmdbuf, "create %d %d", &r, &c)) {
        ...
        handle_list();
    } else if(1 == sscanf(cmdbuf, "print %d", &id)) { //Does not work
        handle_print(id);
