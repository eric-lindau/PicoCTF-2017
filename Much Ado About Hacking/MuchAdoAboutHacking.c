...
void p1(char* message)
{
  printf("%s: %d\n", message, first_person->value);
}

void p2(char* message)
{
  printf("%s: %d\n", message, second_person->value);
}
...
int main(void)
{
  ...
  printf("achilles->value: %d\n", achilles->value); //achilles
  printf("cleopatra->value: %d\n", cleopatra->value); //achilles
  
  act_i_scene_ii:                         /* Benedick strengthens his memory */
  activate_character(49, beatrice);
  char_input(47, second_person); //INPUT
  ...
  pop(60, second_person); //Gets rid of space
  ...
  p2("beatrice+=32");
  char_output(93, second_person); //OUTPUT
